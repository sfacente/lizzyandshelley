...
#(WHY IS THERE AN ERROR HERE: Error: no valid set of coefficients has been found: please supply starting values)
```
...
#(WHY IS THERE AN ERROR HERE: Error in glm.control(...) : unused argument (offset = df$SUMINJ))
```

Compare poisson and negative binomial models.
```{r}
lrtest(modelpoisson, modelnb_categorical)
AIC(X,Y)
```
