...
Descriptive variables for stratifying cascades, bivariate, and multivariate analyses:
```{r}
#city
...
REVISE AND RESUBMIT: Treatment (potentially creating a variable, can add to existing section above if we are going to include it in analyses.)
```{r}
...
#CURTXA: Current Methadone detoxification
table(df$CURTXA, useNA = "ifany")

#CURTXB: Current Methadone Maintenance
table(df$CURTXB, useNA = "ifany")

#CURTXC: Current Buprenorphine
table(df$CURTXC, useNA = "ifany")

#CURTXD: Current Outpatient treatment
table(df$CURTXD, useNA = "ifany")

#CURTXE: Current Inpatient hospital treatnt
table(df$CURTXE, useNA = "ifany")

#CURTXF: Current Residential treatment
table(df$CURTXF, useNA = "ifany")
```

REVISE AND RESUBMIT: Were there statistically significant differences in proportions at each step in the engagement cascades stratified by city and race/ethnicity?  Re-run these since answers seem kind of weird.
```{r}
#differences by city
...
myVars <- c("LOCALID", "GENDER","agecat","RACE_cat", "ED","HOMELESS", "JAIL", "STIM.freq", "OPIOID.freq", "WOD6M", "ODP6M") 

# Specify which of the variables are categorical
catVars <- c("LOCALID", "GENDER", "agecat", "RACE_cat", "ED","HOMELESS", "JAIL", "STIM.freq", "OPIOID.freq", "WOD6M", "ODP6M") 
...
#glm(NAL6MOS.new ~ factor(LOCALID) + factor(GENDER) + factor(agecat) + factor(ED) + factor(HOMELESS) + factor(JAIL) + factor(STIM.freq) + factor(OPIOID.freq) + factor(WOD6M) + factor(ODP6M), data=df, family=binomial(log))
```
...
df.modpoiss <- subset(df, select=c(SUBJECT, NAL6MOS.new, LOCALID, GENDER, agecat, RACE_cat, ED, HOMELESS, JAIL, STIM.freq, OPIOID.freq, WOD6M, ODP6M))
...
fitnal6mos.modpoiss  <- geeglm(formula=NAL6MOS.new ~ factor(LOCALID) + factor(GENDER) + factor(agecat) + factor(RACE_cat) + factor(ED) + factor(HOMELESS) + factor(JAIL) + factor(STIM.freq) + factor(OPIOID.freq) + factor(WOD6M) + factor(ODP6M), data=df.modpoiss, id=SUBJECT, family=poisson(link="log"), corstr = "exchangeable")
...
MOVING ON TO CURRENTLY OWNING NALOXONE....
...
df.modpoiss.nalown <- subset(df_nal6mos, select=c(SUBJECT, NALOWN, LOCALID, GENDER, agecat, RACE_cat, ED, HOMELESS, JAIL, STIM.freq, OPIOID.freq, WOD6M, ODP6M))
