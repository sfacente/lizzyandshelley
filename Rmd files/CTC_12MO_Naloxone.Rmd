...
df$ANYOPIATEUSE <- ifelse(df$HRNINJ=="Yes" |df$OPNINJ=="Yes" | df$HRINJ=="Yes" | df$OPINJ=="Yes", 1, ifelse(df$HRNINJ=="No" & df$OPNINJ=="No" & df$HRINJ=="No" & df$OPINJ=="No", 0, NA))
...
myVars <- c("LOCALID", "GENDER","agecat","RACE_cat", "ED","HOMELESS", "JAIL", "ANYSTIMUSE", "ANYOPIATEUSE", "WOD6M", "ODP6M") 

# Specify which of the variables are categorical
catVars <- c("LOCALID", "GENDER", "agecat", "RACE_cat", "ED","HOMELESS", "JAIL", "ANYSTIMUSE", "ANYOPIATEUSE", "WOD6M", "ODP6M") 
...
#glm(NAL6MOS.new ~ factor(ANYOPIATEUSE), data=df, family=binomial(log))
#glm(NAL6MOS.new ~ factor(WOD6M), data=df, family=binomial(log))
#glm(NAL6MOS.new ~ factor(ODP6M), data=df, family=binomial(log))

#glm(NAL6MOS.new ~ factor(LOCALID) + factor(GENDER) + factor(agecat) + factor(ED) + factor(HOMELESS) + factor(JAIL) + factor(ANYSTIMUSE) + factor(ANYOPIATEUSE) + factor(WOD6M) + factor(ODP6M), data=df, family=binomial(log))
```
