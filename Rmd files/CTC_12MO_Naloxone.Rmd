...
Descriptive variables for multivariate analyses:
```{r}
#city
...
Table 1 for correlates of ever receiving naloxone [NALEVER].
```{r}
...
Bivariate and multivariate models for ever receiving naloxone using logistic regression (because log-binomial models wouldn't converge for all variables).
```{r}
...
Bivariate and multivariate models for ever receiving naloxone using modified Poisson regression with robust standard errors. (CITE ZOU'S MODIFIED POISSON 2004 paper)
```{r}
require(geepack)
...
If I end up needing to calculate the 95% CI for RRs by hand instead.
```{r}
...
Table 1 for correlates of currently owning naloxone [NALOWN].
DEAL WITH THIS ONCE WE ADDRESS THE SKIP PATTERN ISSUE.
```{r}
...
```{r}
#can't get ifelse to work
#df$NAL6MOS.new <- ifelse((df$NALEVER=="Yes" & df$NAL6MOS=="Yes"), 1, ifelse(df$NALEVER=="Yes" & df$NAL6MOS=="No"), 0, ifelse(df$NALEVER=="No"), 0)

#corrected ifelse
df$NAL6MOS.new <- ifelse(df$NALEVER==1 & df$NAL6MOS==1, 1, ifelse(df$NALEVER==1 & df$NAL6MOS==0, 0, ifelse(df$NALEVER==0, 0, NA)))
