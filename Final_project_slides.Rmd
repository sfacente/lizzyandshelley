...
#
## Title: \normalfont{**SNARKY: Sharing Needles As a Result of Kowtowing to Yuppies**}

## Authors: \normalfont{Elizabeth Kinnard and Shelley Facente}
...
\vspace{20pt}
Among people who inject drugs (PWID), is frequency of public injection (e.g. on a sidewalk) associated with increased frequency of injecting with a syringe previously used by someone else?
...
bingeb <- sum(baseline$binge)
bingebp <- bingeb/nrow(baseline)*100
binge1 <- sum(FU1$binge1)
binge1p <- binge1/nrow(FU1)*100
binge2 <- sum(FU2$binge2)
...
| \fontcolor{white}{.}| \fontcolor{white}{.} | Baseline | 6-month F/U | 12-month F/U |
| ----------- | -------- | --------- | --------|
...
# Statistical Model
We converted our research question into a parameter of interest using a series of longitudinal regression models with the following form:

\begin{align*}
log (E[Y|\textbf{X}]) &= \beta_0 + \beta_{0i} + (\beta_1 + \beta_{1i} + \beta_{1j}) X_{1ij} + (\beta_2 + \beta_{2i} + \beta_{2j}) X_{2ij} + \beta_3X_3 + \beta_4X_4 + \\ 
&\ \ (\beta_5 + \beta_{5i} + \beta_{5j}) X_{5ij} + (\beta_6 + \beta_{6i} + \beta_{6j}) X_{6ij} + (\beta_7 + \beta_{7i} + \beta_{7j}) X_{7ij}
\end{align*}
...
exp(modelgee_categorical$coefficients)
...
###############
# NEED TO ESTIMATE CONFIDENCE INTERVALS FOR ALL 3 MODELS USING SANDWICH ESTIMATOR
###############
```
...
AIC(modelpoisson_categorical, modelnb_categorical, modelgee_categorical)
```
