...
```{r hist, include = TRUE, echo = FALSE, warning = FALSE, message = FALSE, fig.height = 4}
...
hist2 <- ggplot(subset(dfslong, dfslong$TIMESHARE>0), aes(x=TIMESHARE)) + geom_histogram(aes(y=..density..), colour="black", fill="white")+ geom_density(alpha=.2, fill="darkblue")+ scale_x_continuous(limits = c(0, 100)) + xlab("Number of times sharing used syringes") + ylab("Density") + ggtitle("B")
...
\footnotesize
We can see from these results that the inference is similar across models, though the magnitude of the effect is much stronger in the negative binomial model, and the standard error is much larger for the GEE model.

```{r graph, include = FALSE, eval = FALSE}

cbPalette <- c("#999999", "#E69F00", "#56B4E9", "#009E73", "#F0E442",
               "#0072B2", "#D55E00", "#CC79A7")

plot_to_print <- ggplot(data = dfslong, aes(x = VISIT, y = TIMESHARE,
                            group=interaction(VISIT, PUBINJ_3cat))) +
  geom_boxplot(aes(fill=factor(PUBINJ_3cat))) + scale_fill_manual(values=cbPalette) +
  ggtitle("Outcome over time: syringe sharing by public injection frequency") +
  labs(y= "Times shared", x = "Public Injecting") ; plot_to_print
