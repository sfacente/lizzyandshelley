...
# Attempt a compelling graph related to your research question
...
##################
#trying alternative method using predict function

#model_p <-predict(modelnb_categorical)
#model_p <- as.data.frame(model_p)

#Error in data.frame(dfslong$SUBJECT, dfslong$PUBINJ_3cat, model_p) : arguments imply differing number of rows: 2934, 1069
#predicted_df <- data.frame(dfslong$SUBJECT, dfslong$PUBINJ_3cat, model_p)
#colnames(predicted_df) <- c("subject", "pubinj", "predicted_y")

# modify this code if i can get it to work
#gg_nb <- ggplot(data = predicted_df, aes(x = time, y = predicted_y, group=id)) +
  #geom_line(aes(color=factor(id))) + theme_classic() +
  #ggtitle("Strength over Time, Predicted for Each Individual") +
  #labs(y= "Lin Mod Prediction of CD4 ", x = "Time") +
  #facet_grid(. ~ drug.base) +
