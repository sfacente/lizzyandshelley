...
#gee
dfslong <- subset(dfslong, SUMINJ>0)
modelgee_categorical <- gee(TIMESHARE ~ PUBINJ_3cat + SUMINJ + factor(PUBINJ_baseline) + GENDER + AGE + ANYSTIM + ANYOPIATE + binge, SUBJECT, data = dfslong, family = poisson, corstr = "exchangeable", na.action = na.omit)
...
summary(modelnb_categorical1)
NB.pvalue <- data.frame(summary(modelnb_categorical1)$coefficients)
NB.est <- data.frame(exp(summary(modelnb_categorical1)$coefficients))
NB.out <- data.frame(Est.MM.NB=NB.est[,1], pvalue.MM.NB=NB.pvalue[,4]); NB.out
NBrand <- as.data.frame(VarCorr(modelnb_categorical1))
