...
#gFU1 <- glm(FU1 ~ GENDER, family = "binomial", data = df) ####FAKETOTEST####
gFU1 <- glm(FU1 ~ x + y + z, family = "binomial", data = df)
#gFU2 <- glm(FU2 ~ FU1 + GENDER, family = "binomial", data = df) ####FAKETOTEST####
gFU2 <- glm(FU2 ~ FU1 + x + y + z, family = "binomial", data = df) 
...
dfs <- df %>%
  select("GENDER", "AGE", "RACELG", "HOMELESS", "HOMELESS1", "HOMELESS2", "MOSHOME", "MOSHOME1", "MOSHOM2", "NVHOUSED", 
         ...
         "NALAVAIL1", "POLICE1", "POLICE2", "POLTIME1", "ARREST1", "ARREST2", "JAIL1", "JAIL2", "TX6MOS1", "TX6MOS2", "w")
```
...
#examples of analysis using reweighted sample
model <- glm(Y ~ X, weights = w, data = dfs, family = "binomial", na.action = na.omit)
#model <- glm(HLP6M2 ~ JAIL1 + JAIL2, weights = w, data = dfs, family = "binomial", na.action = na.omit) ####FAKETOTEST####
summary(model)
```
